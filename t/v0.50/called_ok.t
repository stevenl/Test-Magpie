...
like(
    $e = exception {
        called_ok( sub { $mock->once }, tiny => 1 );
    },
...
like(
    $e = exception {
        called_ok( sub { $mock->once }, times => 'string' );
    },
...
like(
    $e = exception {
        called_ok( sub { $mock->twice }, at_least => 'once' );
    },
...
like(
    $e = exception {
        called_ok( sub { $mock->twice }, at_most => 'thrice' );
    },
...
like(
    exception {
        called_ok( sub { $mock->twice }, between => 1 );
    },
...
like(
    exception {
        called_ok( sub { $mock->twice }, between => [ 'one', 'two' ] );
    },
    qr/'between' option must be an arrayref with 2 numbers in ascending order/,
...
like(
    $e = exception {
        called_ok( sub { $mock->twice }, between => [ 2, 1 ] );
    },
    qr/'between' option must be an arrayref with 2 numbers in ascending order/,
...
like(
    $e = exception {
        called_ok( sub { $mock->thrice( SlurpyArray, 1 ) } );
    },
...
like(
    $e = exception {
        called_ok( sub { $mock->once( SlurpyArray, 1 ) } );
    },
    qr/^No arguments allowed after a slurpy type constraint/,
    'Disallow arguments after a slurpy type constraint'
);
...
like(
    $e = exception {
        called_ok( sub { $mock->thrice( slurpy Any ) } );
    },
