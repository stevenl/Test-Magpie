...
SKIP: {
    ...
    test_out( qr/ok 1 - set\(StrMatch\[\(\?.*:\^foo\)\]\) was called 2 time\(s\)\s?/ );
...
SKIP: {
    ...
    test_out( qr/ok 1 - set\(StrMatch\[\(\?.*:\^foo\)\]\&StrMatch\[\(\?.*:bar\$\)\]\) was called 1 time\(s\)\s?/ );
    called_ok(
...
called_ok( sub { $mock->set( slurpy Tuple[Defined,Defined] ) }, times => 2 );
...
called_ok( sub { $mock->set( slurpy Map[$positive_int, Str] ) }, times => 1 );
