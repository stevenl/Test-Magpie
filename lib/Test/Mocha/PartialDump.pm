...
Test::Mocha::PartialDump - Partial dumping of data structures, optimized for argument
printing.
...
=head1 SYNOPSIS
	...
	sub foo {
		print "foo called with args: " . Test::Mocha::PartialDump->new->dump(@_);
	}

	use Test::Mocha::PartialDump qw(warn);
...
to get more informative errors during development, or even use it as:
	...
	BEGIN { local $@; eval "use Test::Mocha::PartialDump qw(...)" }
...
These methods will use C<$Test::Mocha::PartialDump::default_dumper> as the invocant if the
first argument is not blessed and C<isa> L<Test::Mocha::PartialDump>, so they can be
